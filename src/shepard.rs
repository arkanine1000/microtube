...
const F_MIN: f64 = 32.703_195_662_574_83;
...
impl ShepardEngine {
    ...
    pub fn process(&mut self, direction: Direction, intensity: f64) -> f64 {
        if intensity <= 0.0 {
            return 0.0;
        }
        ...
        for i in 0..NUM_OCTAVES {
            ...
            let freq = F_MIN * (2.0_f64).powf(self.log_offsets[i]);
...
mod tests {
    ...
    fn offsets_stay_in_range() {
        ...
        for _ in 0..(48_000 * 60) {
            ...
            let _ = engine.process(Direction::Up, 1.0);
        }
    ...
    fn output_bounded_at_full_intensity() {
        ...
        for _ in 0..(48_000 * 4) {
            let s = engine.process(Direction::Up, 1.0).abs();
            if s > peak {
                peak = s;
            }
        }
    ...
    fn zero_intensity_silent() {
        let mut engine = ShepardEngine::new(48_000.0);
        for _ in 0..1024 {
            assert_eq!(engine.process(Direction::Up, 0.0), 0.0);
        }
    }
    ...
    fn offsets_remain_evenly_spaced() {
        ...
        let mut engine = ShepardEngine::new(48_000.0);
        for _ in 0..7919 {
            let _ = engine.process(Direction::Up, 0.5);
        }
